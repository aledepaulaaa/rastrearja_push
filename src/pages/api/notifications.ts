...
async function deleteToken(req: NextApiRequest, res: NextApiResponse) {
    ...
    // Filtra os tokens para remover com base no deviceId ou fcmToken
    if (deviceId) {
        tokens = tokens.filter((t: any) => t.deviceId !== deviceId)
    } else if (fcmToken) {
        tokens = tokens.filter((t: any) => t.fcmToken !== fcmToken)
    } else {
        // Se a requisição não tiver deviceId ou fcmToken, retorna erro
        return res.status(400).json({ error: 'Token ou deviceId necessário para desregistro' })
    }
